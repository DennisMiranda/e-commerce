  ...
  const categories = result.recordset.reduce((acc, row) => {
    ...
    if (!category) {
      acc.push({
        id: row.id,
        name: row.name,
        products: row.product_id ? [{ id: row.product_id, name: row.product_name, price: row.price, stock: row.stock }] : []
      });
    } else {
      if (row.product_id) {
        category.products.push({ id: row.product_id, name: row.product_name, price: row.price, stock: row.stock });
      }
    }
    return acc;
  }, []);
